...
impl Network {
    ...
    pub fn feed_forward(&mut self, inputs: Vec<f64>) -> Vec<f64> {
        ...
        for i in 0..self.layers.len() - 1 {
            current = self.weights[i]
                .multiply(&current)
                .add(&self.biases[i])
                .map(|x| Activation::sigmoid_function(x));
    ...
    pub fn back_propogate(&mut self, outputs: Vec<f64>, targets: Vec<f64>) {
        ...
        let mut gradients = parsed.map(|x| Activation::sigmoid_derivative(x));
        ...
        for i in (0..self.layers.len() - 1).rev() {
            ...
            gradients = self.data[i].map(|x| Activation::sigmoid_derivative(x));
        }
    }
